...
{
    ui->setupUi(this);
    setFixedSize(SET_SERVER_SIZE_X, SET_SERVER_SIZE_Y);
...
void TcpServer::initServer() {
    ...
    bytesWritten = INIT_WRITTEN_BYTES;
    bytesToWrite = INIT_TO_WRITE_BYTES;
...
void TcpServer::updateClientProgress(qint64 numBytes) {
    ...
    if (bytesToWrite > 0) {
        ...
        outBlock.resize(0);
    } else localFile->close();

    ui->progressBar->setMaximum(TotalBytes);
    ui->progressBar->setValue(bytesWritten);

    float useTime = time.elapsed(); //获取耗费时间
